...
点击封面或标题进入漫画详情。桌面左侧显示封面，右侧显示标题、作者、元数据与「阅览」按钮，
窄屏改为单列；下方展示分类标签和来源归档信息，原始数据默认折叠。
返回漫画库时恢复原来的查询条件与页码。详情页只加载封面，「阅览」直接跳转独立阅览器。

「连接设置」提供「阅览器地址」，留空使用自带 `/dmb-viewer.html`，支持本站路径与 HTTP / HTTPS URL。
打开阅览器时设置 `id` 为当前 DMB ID，并保留地址中其他参数；设置保存后立即更新当前详情的阅览链接。
「图片线路」保留代理 / 8880 直连选项，默认代理，只用于 CM 漫画库和详情封面。
保存后刷新当前封面并记住选择，不重置检索输入或未保存的录入选择。

自带独立阅览器在页码组件右侧显示「已缓存 X / N 页」、预加载状态、缓存容量和进度条，页码不固定居中。
失败页显示数量，当前页失败提供「重试本页」，后台队列结束后提供「重试失败页」。
缓存连续加载至整部完成或退出；回翻与 Viewer.js 放大复用已加载图片。
工具栏保持单行、高度 60px（另加底部安全区），窄屏时状态区内部换行，不另占一行。进度更新不持续触发屏幕阅读器播报。
...
列表显示排队中、解析中、下载中和 failed，排除 archived、deleted 和 purged；下载中条目优先。
